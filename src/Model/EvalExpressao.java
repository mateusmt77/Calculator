    ...
    static List<String> token(String expressaoCompleta) {
        ...
        for (int i = 0; i < expressaoCompleta.length(); i++) {
            ...
            if ((Character.isDigit(c)) || (c == '.')) {
                int x = i;
                while (x < expressaoCompleta.length()
                        && (Character.isDigit(expressaoCompleta.charAt(x)) || expressaoCompleta.charAt(x) == '.')) {
                    x++;
                }
        ...
        for (String tokenAtual : tokens) {
            if (isNumber.test(tokenAtual)) {
                ...
                while (!operadores.isEmpty()
                        ...
                        && operadoresPrecedencia.getOrDefault(operadores.peek(), 0) >= operadoresPrecedencia
                                .getOrDefault(tokenAtual, 0)) {
                    saida.add(operadores.pop());
                }
                operadores.push(tokenAtual);
            }
